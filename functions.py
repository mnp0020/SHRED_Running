...
def partition_data_seq(load_X, n, lags):
    ...
    Return:
      ...
      test_indices: remaining indices for testing
    """
    # # Train: keep first 30% and last 30% of n
    # interval10 = int(0.10*n) # 10% of n
    # first30 = np.arange(0, 3*interval10) # first 30% of n
    # last30 = np.arange(7*interval10, n-lags)
    # train_indices = np.concatenate((first30, last30), axis=None)
    
    # # Test: keep inner most 20% of n
    # test_indices = np.arange(4*interval10, 6*interval10)

    # # Valid: keep the rest
    # mask = np.ones(n-lags)
    # mask[train_indices] = 0
    # mask[test_indices] = 0
    # valid_indices = np.arange(0, n)[np.where(mask!=0)[0]]
...
def rmse_error(Ypred, Ytest):
    ...
    """
    # # this should be RMSE?
    # err = np.linalg.norm(Ypred - Ytest) / np.sqrt(len(Ytest)) 

    # trying another way to get rmse
    # err = np.sqrt(mean_squared_error(Ytest, Ypred, squared=False))

    # and yet another way to get RMSE, I trust this 
    err = np.sqrt(((Ypred - Ytest) ** 2).mean()) 

    # this is not RMSE, but may still be meaningful?
    # err = np.linalg.norm(Ypred - Ytest) / len(Ytest)

    # normalized error that Megan used
